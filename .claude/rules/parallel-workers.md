...
- Worker IPC payloads are plain arrays — deserialize them with `@unserialize($blob, ['allowed_classes' => false])` in both `run()` and `runStreaming()`. Never widen to `true`; if a worker must return objects, allow-list them via `Phpdup\Persistence\SerializedClassAllowList` instead of re-enabling all classes.
